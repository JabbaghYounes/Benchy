...
- Hailo backend supports all five YOLO tasks: detection, classification, OBB, segmentation, pose. OBB uses `postprocessing.py:_rotated_nms` / `_process_obb`; segmentation uses `_process_segmentation` / `_generate_seg_masks`; pose uses `_process_pose` with a 17-keypoint COCO-Pose decoder. v8/v11 are verified across all tasks. v26-{obb,pose} are experimental until hardware confirmation on both boards; v26-seg works on Hailo-10H (HEF in hefs-v3) but is chip-incompatible on Hailo-8 — six override variants attempted, none fit the matmul1 capacity (pitfalls.md § 12). The AI HAT+ verify therefore tags v11-seg (pitfalls.md § 11) and v26-seg `[unsupported-on-this-hw]`.
