...
- **HEF compilation is x86_64-only** (Hailo Dataflow Compiler / `hailo_sdk_client` does not run on aarch64). On the Pi the backend therefore looks for prebuilt HEFs in two locations before attempting compilation: `resources/hefs/` (user-supplied workstation builds, named `<version>_<task>_<size>_<arch>.hef` per `resources/hefs/NAMING.txt`; four Hailo Model Zoo v2.16.0 HEFs are already staged there — `v8_detection_n_hailo8.hef`, `v11_detection_n_hailo8.hef`, `v8_segmentation_n_hailo8.hef`, `v8_pose_s_hailo8.hef`) and `/usr/share/hailo-models/` (the curated subset shipped by `rpicam-apps-hailo-postprocess`). See `benchmark/workloads/yolo/conversion/hef_source.py` and Issue 11 in `resources/session_issues_2026-04-27.md`. To extend coverage of the system-package map, only add entries that physically exist on disk — `tests/test_hef_source.py:test_system_package_map_only_contains_observed_filenames` will fail otherwise.
...
- **Workstation compile path internals.** End-node truncation is mandatory for YOLO heads — Hailo's parser can't ingest the DFL/Reshape/Cos tail, so `benchmark/workloads/yolo/conversion/har_generator.py:END_NODE_TABLE` keys `(yolo_version, YOLOTask)` to the per-task end-node lists captured from real Hailo diagnostics (v8/v11 detection + v11 seg / pose seeded; v26 + v8 non-detection rely on the `parse_end_node_hint` fallback that re-runs the SDK with names parsed from its own error message). `pipeline.py:_run_har_generation` populates `HARGeneratorConfig.end_nodes` from the table; on table miss the diagnostic-hint fallback covers it. Calibration data is stacked to `(N, H, W, C)` numpy before `runner.optimize` (a list of `(H, W, C)` arrays trips Hailo's type detector with "Couldn't detect CalibrationDataType"). The compile CLI's `--calibration-set-size` defaults to 1024 (Hailo's threshold for Bias Correction passes) and `--calibration-data-path` lets you stage val2017 (~1 GB) instead of letting the loader auto-download all of coco (~27 GB). The calibration cache key embeds the dataset identity so changing `DEFAULT_DATASETS` or `--calibration-data-path` invalidates stale `.npz` files automatically. **Gap-model compiles (seg/pose/OBB) require an NVIDIA GPU** — without CUDA, Hailo's optimizer drops to level 0, biases stay 16-bit, and chip mapping fails on Hailo-8 with `DW resources / 16x4 not supported in activation2`. CPU compiles work for plain detection but not for seg/pose/OBB. See `docs/compilation/nvidia_workstation_setup.md` for the bring-up checklist and `docs/compilation/end_node_truncation_plan.md` for the truncation table rationale.
...
- `session_issues_<YYYY-MM-DD>.md` — dated incident logs from setup-and-verify sessions on real hardware. The 2026-04-27 entry documents twelve issues fixed during the AI HAT+ Pi bring-up: 1-5 setup hygiene (HailoRT 5.x stray, root-owned log/egg-info, venv `hailo_platform` symlink bug, missing `[dev]` extras), 6-7 LLM model-surface consolidation (1B/3B/7B llama-only), 8-10 from the first end-to-end verify run (missing `onnx`/`onnxruntime` deps killing the YOLO Hailo conversion pipeline; LLM HTTP timeout too short for cold-loaded `llama2:7b` on Pi 5; `csv_writer.py` LLM column list out of sync with `LLMAggregatedMetrics.to_dict()`), and 11-12 from the second verify run (Hailo Dataflow Compiler unavailable on aarch64 → added prebuilt HEF source layer at `benchmark/workloads/yolo/conversion/hef_source.py`; even 600s timeout couldn't cover llama2:7b cold-load → added pre-warm step + made HTTP timeout YAML-configurable). Read the relevant file before debugging Hailo-stack, venv, LLM-config, or report-pipeline failures — most failure modes are already catalogued there with root cause and verified fix.
