...
now implemented and populated for v8/v11/v26 detection + OBB,
v8/v11/v26 segmentation, and v11/v26 pose — covering 11 of the 15
HAILO_SUPPORTED (version, task) combinations after the 2026-04-30
retry sweep on the NVIDIA workstation.
...
- **`benchmark/workloads/yolo/conversion/har_generator.py:END_NODE_TABLE`**
  — populated entries: `(v8, det)`, `(v8, seg)`, `(v8, obb)`,
  `(v11, det)`, `(v11, seg)`, `(v11, pose)`, `(v11, obb)`,
  `(v26, det)`, `(v26, seg)`, `(v26, pose)`, `(v26, obb)`. Remaining
  gap entries: `(v8, pose)` and classification across all three
  versions.
- **`docs/compilation/pitfalls.md` § 10** — the rule for adding new
  entries (cut at raw Conv outputs; verify against Hailo Model Zoo
  YAMLs; cross-check against `postprocessing.py`). For v26
  specifically, head conv names are prefixed `one2one_cv*` (the
  one-to-one matching head Ultralytics introduced in v26); v26 pose
  also uses a flatter `one2one_cv4_kpts.X/Conv` layout.
- **`resources/session_notes_2026-04-29_nvidia_workstation.md`** —
  full forensics on why the deep-layer cut was wrong (Issue 6),
  the procedure for deriving missing entries (Issue 10), and the
  retry sweep that filled in OBB + v26 entries from ONNX inspection
  and Hailo Model Zoo YAML cross-reference.
