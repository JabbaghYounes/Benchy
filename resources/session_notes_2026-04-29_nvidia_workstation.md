...
2. **Cache-key arch fix** (Issue 9) — **DONE on AMD workstation.** `cache.get_cache_path` now requires `target_device` (keyword-only); on-disk layout is `models/hailo/<arch>/<version>/<task>/<model>/`. The 24 call sites in `pipeline.py`, `hef_compiler.py`, `har_generator.py`, `onnx_export.py` all thread `config.target_device` through. `tests/test_cache_arch_isolation.py` is the regression test: compiles a fake model for both archs, asserts paths differ AND md5 differs AND `has_hef(...arch=X)` doesn't false-positive on arch=Y's cache. Pre-existing `models/hailo/` directories from before the fix are silently ignored (cache miss → recompile); clear if disk space matters. Multi-arch sweeps no longer require manually clearing `models/hailo/` between archs.
